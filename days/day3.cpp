...
namespace aoc
{
	void Day3::run()
	{
		...
		std::array<int, 26 * 2> group_counts;
		group_counts.fill(0);
		int group_index = 0;

		std::array<int, 26 * 2> char_counts_in_left;
		std::array<int, 26 * 2> char_counts_in_right;
		std::array<bool, 26 * 2> added_for_this_bag;

		while (std::getline(file, line))
		{
			assert(line.size() % 2 == 0);
			const int half_length = line.size() / 2;
			...
			{
				char_counts_in_left[compartment1[i] - (compartment1[i] >= 'a' ? 'a' : 'A' - 26)] += 1;
				char_counts_in_right[compartment2[i] - (compartment2[i] >= 'a' ? 'a' : 'A' - 26)] += 1;
			}
			...
			if (group_index >= 3)
			{
				...
				int a = std::distance(group_counts.begin(), r) + 1;
				group_badge_total_ += std::distance(group_counts.begin(), r) + 1;
